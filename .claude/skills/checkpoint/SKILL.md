---
name: checkpoint
description: Human gate at epic completion — demo instructions, QA summary, remap proposal, batched questions; records the human decision, applies remaps, starts the next epic. Usage - /checkpoint EP-##
---

# /checkpoint — human gate per epic

Argument: epic ID (default: current epic). Precondition: epic-level QA verdict is
PASS/PASS-WITH-NOTES — a FAIL cannot reach checkpoint.

1. Prepare `project/05-epics/EP-##/checkpoint.md` from `templates/checkpoint.md`:
...
3. Record the decision in checkpoint.md §6 + state.yaml history:
   - **Approved:** epic → `done`; apply Q answers; run `/trace`; `current_epic` → next;
     commit `EP-##: checkpoint approved`; immediately run `/epic <next>`.
   - **Approved with changes:** as above, plus changes become tasks in the next epic or a
     remap.
   - **Rework:** epic back to `building` with the rework list routed via the planner.
4. Apply approved remaps: dev-plan epic map + remap log updated, affected `pending` epics
