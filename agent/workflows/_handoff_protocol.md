...
Orchestrator-driven (`agent/workflows/handoff-freeze.md`):
1. WIP-commit the branch (`wip(E03-T07): freeze for handoff`).
2. Write the packet `agent/handoffs/<id>.yaml` from the template —
...
7. Non-obvious choices get a Decision record (one line minimum) in
   `agent/memory/decisions/` so future agents don't re-litigate.
