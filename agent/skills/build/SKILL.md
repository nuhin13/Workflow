...
2. **Dispatch.** For each picked task, follow
   `agent/workflows/implement-task.md`: branch `epic_<NN>_task_<MM>`, one
   ...
   `agent/adapters/run-<platform>.sh` so cost + session land in `runs/` and
   `metrics.csv`.
...
4. **Peer review gates the task PR.** `reviewed_by` ≠ `executed_by`
   (`agent/workflows/_handoff_protocol.md` §2); merge into the epic branch on
