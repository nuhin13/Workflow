---
name: qa
description: Run the independent QA agent against a task or epic — fresh context, spec + repo only, no builder reasoning; produces a PASS/FAIL QA report; failures route back through the planner. Usage - /qa E<NN> | /qa E<NN>-T<MM>
---
...
4. **On FAIL:** status → `changes-requested`. Route findings to the **planner**: spec defect →
