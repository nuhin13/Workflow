...
2. Diff ONLY the harness paths (`harness.yaml: harness_change_policy.protected`
   plus `agent/`, `Makefile`, `requirements.txt`):
   `git diff HEAD template/main -- agent/ AGENTS.md CLAUDE.md templates/ harness.yaml epics/_templates/ Makefile requirements.txt`
...
2. Branch the template repo. Apply:
   - the lesson entry into the template's `agent/memory/lessons/<area>.md`
...
- Service dependencies (MCP servers, DB, Figma): those are per-project
  config (`.mcp.json`, `.env`) — copy the *examples* from `agent/mcp/`,
