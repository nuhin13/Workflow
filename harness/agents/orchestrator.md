...
- Budgets: warn at `per_task_warn_usd` / `per_epic_warn_usd`; stamp every run
  into `metrics.csv` via `harness/orchestrator/metrics_collect.py`.
...
- Rate-limit watch: on freeze signal (skills/rate-limit-handoff), run
  `harness/workflows/handoff-freeze.md`, then resume on the next platform in
  `harness.yaml: platforms`.
- Mid-stream human input: route via `harness/workflows/inject-work.md`.
