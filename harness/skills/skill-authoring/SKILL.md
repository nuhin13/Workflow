...
Skills live in `harness/skills/<name>/SKILL.md`. They are loaded ON DEMAND —
...
1. Create `harness/skills/<name>/SKILL.md`.
2. Add it to the `skills:` list of every role card in `harness/agents/` that
   should auto-load it.
3. Reference it from the workflow step where it applies
   (`harness/workflows/*.md`) — workflows carry the ORDER, skills the DEPTH.
