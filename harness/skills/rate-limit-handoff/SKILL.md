...
- Claude Code: statusline JSON exposes `rate_limits` (five_hour, seven_day) —
  wired via `harness/hooks/statusline-ratelimit.sh`; also `/usage` and
...
- Freeze threshold: **80% of the 5h window**, or weekly headroom < the next
  ...
  `python3 harness/orchestrator/ratelimit_guard.py` (reads statusline JSON on
...
2. Write the packet from `harness/handoffs/_template.handoff.yaml` →
   `harness/handoffs/E03-T07.yaml` + save `git diff origin/epic_03` alongside.
