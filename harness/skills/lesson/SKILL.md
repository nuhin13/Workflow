---
name: lesson
description: Append a lesson learned (L-<area>-###) to harness/memory/lessons/ — after QA failures, checkpoint surprises, rework, or any "we should never do that again" moment. Lessons bind future planning and building.
---
...
1. Take the lesson from `$ARGUMENTS` or the current situation (a QA report's proposals, a
   remap's cause).
2. Pick the area file `harness/memory/lessons/<area>.md` (backend, frontend, git, qa,
